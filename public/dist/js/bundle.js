...
var app = angular.module('app', ['ui.router', 'ui.materialize', 'ngAnimate']).config(function ($stateProvider, $urlRouterProvider) {
...
app.controller('homeCtrl', function ($scope, mainService, $state) {
  ...
  function getUser() {
    mainService.getUser().then(function (user) {
      ...
      if (user) $scope.currentUser = user;else $scope.currentUser = 'NOT LOGGED IN';
    });
  }
  getUser();
...
app.controller('mainCtrl', function ($scope, mainService, $state) {
  ...
  $scope.addBookToUserBooks = function (info) {
    var book = {
      user_id: $scope.currentUser.id,
      book_id: book.bid,
  ...
  function getUser() {
    mainService.getUser().then(function (user) {
      ...
      if (user) $scope.currentUser = user;else $scope.user = 'NOT LOGGED IN';
    });
  }
...
app.controller('profCtrl', function ($scope, mainService, $state) {
  ...
  $scope.removeFromUserBooks = function (id) {
    mainService.removeFromUserBooks(id);
    $state.reload();
  };

  $scope.findUsers = function (id) {
  ...
  $scope.postMessage = function (convo) {
    console.log(convo);
    var newMessage = {
    ...
    $scope.convos[$scope.convos.indexOf($scope.convos.find(function (x) {
      return x.person === convo.person;
    }))].conversation.push({
      mid: $scope.convos.find(function (x) {
        return x.person === convo.person;
      }).conversation[0].mid + 1,
      msg: convo.msg,
      sender: true
    });
    convo.msg = "";
  ...
  function getMessages() {
    mainService.getMessages($scope.currentUser.id).then(function (res) {
      ...
      for (var i = 0; res.length > i; i++) {
        var found = false;
        for (var j = 0; $scope.convos.length > j; j++) {
